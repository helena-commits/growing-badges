...
const Index = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-secondary/30 to-accent/20">
      <div className="max-w-7xl mx-auto space-y-8 p-4">
        ...
        <div className="text-center space-y-6">
          ...
          color: '#9b26b5'
        }}>Faça upload da sua foto, preencha seus dados e baixe seu crachá</p>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <Card className="shadow-elegant border-primary/10 bg-card/80 backdrop-blur-sm">
            <CardHeader>
              <CardTitle style={{
              color: '#0b233f'
            }}>Dados do Crachá</CardTitle>
            </CardHeader>
            <CardContent className="space-y-6">
              {/* Photo Upload */}
              <Uploader bucket="photos" accept="image/png,image/jpeg,image/jpg" onUpload={handlePhotoUpload} label="Foto do Funcionário" preview />

              {/* Name */}
              <div>
                <Label htmlFor="name" className="text-foreground font-medium">Nome</Label>
                <Input id="name" placeholder="Nome do funcionário" value={name} onChange={e => setName(e.target.value)} className="h-12 text-base border-primary/20 focus:border-primary/40 bg-background/80" />
              </div>

              {/* Role */}
              <div>
                <Label htmlFor="role" className="text-foreground font-medium">Função</Label>
                <Input id="role" placeholder="Função desempenhada" value={role} onChange={e => setRole(e.target.value)} className="h-12 text-base border-primary/20 focus:border-primary/40 bg-background/80 w-full overflow-hidden" />
              </div>

              {/* Save to History */}
              <div className="flex items-center space-x-2">
                <Checkbox id="save-history" checked={saveToHistory} onCheckedChange={checked => setSaveToHistory(checked as boolean)} />
              ...
              <div className="space-y-4">
                <Button onClick={handleGeneratePreview} disabled={!isFormValid} variant="premium" size="lg" className="w-full">
                  <Eye className="w-4 h-4 mr-2" />
                  Gerar Pré-visualização
                </Button>

                <Button onClick={handleDownload} disabled={!canDownload || loading} variant="outline" size="lg" className="w-full">
          ...
          <Card className="shadow-elegant border-primary/10 bg-card/80 backdrop-blur-sm">
            <CardHeader>
              <CardTitle style={{
              color: '#9b26b5'
            }}>Pré-visualização</CardTitle>
            </CardHeader>
            <CardContent>
              {isFormValid ? <BadgePreview ref={badgePreviewRef} template={template} photoFile={photoFile} photoUrl={photoUrl} name={name} role={role} onRender={setCanDownload} /> : <div className="border-2 border-dashed border-muted-foreground/25 rounded-lg p-8 text-center">
                  <div className="space-y-4">
